...
class GameTableArea extends StatelessWidget {
  final String mode;
      ...
      child: Column(
        children: [
          Obx(
            () => Row(
              children: [
                GameStatusChip(label: mode),
                const SizedBox(width: 8),
                GameStatusChip(
                  label: '回合 ${controller.round.value}/${controller.maxRound}',
                ),
                const SizedBox(width: 8),
                GameStatusChip(label: '底注 ${controller.baseBet}'),
                const SizedBox(width: 8),
                GameStatusChip(label: '当前注 ${controller.currentBet.value}'),
                const Spacer(),
                GameStatusChip(label: '底池 ${controller.pot.value}'),
                const SizedBox(width: 8),
                GameStatusChip(label: '倒计时 ${controller.countdown.value}s'),
              ],
            ),
          ),
          const SizedBox(height: 8),
          Expanded(
            child: GameTableSurface(controller: controller),
          ),
          const SizedBox(height: 2),
          SizedBox(
            height: 110,
            child: GameHandCardRow(
              owner: '你的手牌',
...
class GameTableSurface extends StatelessWidget {
  final GameController controller;
  const GameTableSurface({
    required this.controller,
  });
  @override
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Stack(
        children: [
          ...
          const Positioned.fill(child: GameTableRim()),
          const Center(child: GameTableCore()),
          Obx(
            () => Align(
              alignment: Alignment.topCenter,
              child: Padding(
                padding: const EdgeInsets.only(top: 8),
                child: GameSeatWithCards(
          ...
          Obx(
            () => Center(
              child: Column(
                ...
                children: [
                  GamePotWidget(amount: controller.pot.value),
                  const SizedBox(height: 6),
                  GameStatusChip(label: controller.statusText.value),
                ],
              ),
            ),
          ),
...
class GameTableRim extends StatelessWidget {
  const GameTableRim();
  @override
  Widget build(BuildContext context) {
...
class GameTableCore extends StatelessWidget {
  const GameTableCore();
  @override
  Widget build(BuildContext context) {
    return IgnorePointer(
...
class GameMiniButton extends StatelessWidget {
  final String label;
  final VoidCallback onPressed;
  const GameMiniButton({
    required this.label,
    required this.onPressed,
  });
  @override
...
class GameTableSeat extends StatelessWidget {
  final String name;
  final int chips;
  final bool isActive;
  final bool isFolded;
...
class GameSeatWithCards extends StatelessWidget {
  final PlayerModel player;
  final bool isActive;
  final bool showFace;
  final double cardWidth;
  final double cardHeight;
  const GameSeatWithCards({
...
class GameSeatCards extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var index = 0; index < 3; index++) {
      ...
      if (cards.length <= index) {
        items.add(GameCardBack(width: cardWidth, height: cardHeight));
      } else if (showFace) {
        items.add(
          GameCardFace(
            card: cards[index],
...
class GameTableSeatCompact extends StatelessWidget {
  final String name;
  final int chips;
  final bool isActive;
  const GameTableSeatCompact({
    required this.name,
    required this.chips,
    this.isActive = false,
  });
...
class GamePotWidget extends StatelessWidget {
  final int amount;
  const GamePotWidget({
...
class GameHandCardRow extends StatelessWidget {
  final String owner;
  final GameController controller;
  const GameHandCardRow({
    required this.owner,
    required this.controller,
  });
  @override
              ...
              Row(
                children: () {
                  ...
                  for (var index = 0; index < 3; index++) {
                    Widget cardWidget;
                    if (cards.length <= index) {
                      cardWidget = const GameCardBack();
                    } else if (!showFace) {
                      cardWidget = const GameCardBack();
                    } else {
                      cardWidget = GameCardFace(card: cards[index]);
                    }
    ...
    return Container(
      ...
      child: Image.asset(
        ...
        errorBuilder: (context, error, stackTrace) {
          return GameCardBack(width: width, height: height);
        },
      ),
    );
  }
}
class GameBackgroundLayer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: const [
        ...
        Positioned.fill(child: GameNoiseTexture()),
      ],
    );
  }
}
class GameNoiseTexture extends StatelessWidget {
  const GameNoiseTexture();
  @override
  Widget build(BuildContext context) {
    return Opacity(
      opacity: 0.08,
      child: CustomPaint(
        painter: GameNoisePainter(),
      ),
    );
  }
}
class GameNoisePainter extends CustomPainter {
  @override
...
class GameFeltTexture extends StatelessWidget {
  const GameFeltTexture();
  @override
  Widget build(BuildContext context) {
    return Positioned.fill(
      child: CustomPaint(
        painter: GameFeltPainter(),
      ),
    );
  }
}
class GameFeltPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
...
class GameGlowCircle extends StatelessWidget {
  final double size;
  final Color color;
  const GameGlowCircle({
    required this.size,
    required this.color,
  });
  @override
